...
for fn in box_exists box_running box_gateway_reachable box_mcp_ready box_ssh_agent_ready host_git_config_get host_git_config_bool host_ssh_commit_signing_enabled box_git_signing_ready box_home_dir project_metadata_file agent_metadata_file record_box_project record_box_agent read_box_agent remove_box_metadata remove_box_home remove_box_artifacts valid_agent agent_binary agent_install_spec codex_platform_install_spec prompt_agent resolve_agent latest_box_seed_file seed_box_file seed_box_claude_config ensure_box_home box_root_mount_source box_home_isolated print_box_home_migration ensure_box ensure_agent agent_version agent_mcp_registered prepare_box_ssh_signing_key carry_git_identity register_mcp node_supported runtime_lock_hash runtime_locked_package_version runtime_package_version runtime_ready ensure_runtime host_terminal_type enter_box gateway_alive gateway_bind_matches gateway_listener_pid gateway_pid_is_ours stop_gateway canonical_path repo_mount_root find_xcode_project_dirs find_swift_package_dirs discover_project_dirs resolve_project_dir require_project_dir project_context_dir legacy_box_name sanitize_name legacy_box_detected gateway_bind_is_loopback container_host_bridge_configured ensure_container_gateway_route; do
