...
status() {
  PATH="$T/fakebin:$PATH" \
    ...
    FAKE_GATEWAY_RC="${FAKE_GATEWAY_RC:-0}" FAKE_MCP_RC="${FAKE_MCP_RC:-0}" FAKE_SSH_RC="${FAKE_SSH_RC:-0}" \
...
echo "status probes OK: container gateway, real MCP session, SSH agent, repair diagnostics"
